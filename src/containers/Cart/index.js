...
const Cart = ({ history }) => {
    ...
    useEffect(() => {
        axios.get(`${ API_ROOT }/orders/current?user_id=${ user_id }`)
    ...
    return (
        <div id="cart" className="wrapper">
            ...
            <div className="content">
                ...
                { order.user_items.map(item => <CartItem key={ item.id } { ...item } />) }
            </div>
            <div className={ (opened ? `` : `closed `) + `summary` }>
                ...
                <div className="summary-content">
                    ...
                    <p className="value">{ order.total_user_items }</p>
                    <p className="title">Tu total:</p>
                    <p className="value">$ { order.total_user_price }</p>
                    <p className="title">Cantidad de Items:</p>
                    <p className="value">{ order.total_items }</p>
                    <p className="title">Total:</p>
                    <p className="value bold color">$ { order.total_price }</p>
                </div>
                { order.total_items > 0 && (
